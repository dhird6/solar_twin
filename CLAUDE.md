...
- **A stimulus you have not located is not a stimulus, and a KPI-03 of 0.00 is the shape a missing one takes.** `SC-05` measured a false-fault rate from a scene where the shadow landed on the ground. Prove it twice before quoting: geometrically with `world/bladeshadow.py` (which panels are covered, and for what fraction of a rotor revolution) and then **in pixels** — `tools/verify_shade.py` for a standing shadow (many panels, one frame each) or `tools/verify_blade_sweep.py` for a moving one (one panel, a whole revolution, the panel its own control). Both score PV-glass pixels against a control, never whole-frame brightness.
- **Geometry is necessary and not sufficient: the sun is 0.53° wide, so a caster narrower than its own penumbra casts no shadow at all.** Measured `SC-14`, 2026-07-31: the swept-disc model claimed 22.9% panel coverage and the pixels showed *nothing* (4.9% dip on the most-covered panel vs 7.1% on a clear control), because a 4 m blade at a 780 m throw has a 7.2 m penumbra. `bladeshadow.max_umbra_distance_m` gives the ceiling — ~432 m for a 4 m blade, while Khavda's nearest real turbine stands at 546 m, so **no real turbine here can cast a hard blade shadow on the modules.** What the same run *did* find is the **tower** shadow: 3 tables darkened in proportion to their distance from the tower's shadow axis, continuously rather than 7% of the time. Prefer it, and prefer tracker self-shading (`SC-11`/`SC-12`) over either.
...
- **The look layer is MDL, not `UsdPreviewSurface`, and the flat default is opt-out.** RTX renders MDL natively and only *translates* `UsdPreviewSurface` — which is why the old textured path's diffuse was "ignored outright". `OmniPBR.mdl`/`OmniGlass.mdl` resolve **by bare name** on this build (no download, no network); `world/mdl_materials.py` owns them and `tools/material_probe.py` renders every look so a silently-defaulted material is visible. `realism.enabled` (farm config, or `--realism on`) swaps flat colours for MDL, rebuilds the module as glass over FLUSH cells with a perimeter frame, and authors the torque tubes and piles — off by default because every recorded KPI was measured against the flat looks. Exposure: only `fNumber`/`cameraShutter`/`filmIso` work (the Reinhard keys are inert), **f/9** is the measured daylight stop, and they must be applied AFTER `open_stage` or the renderer resets them. Full measurements in `docs/ENVIRONMENT.md`.
- **No NVIDIA library asset exists for a PV module, tracker or inverter** — checked against the Isaac 6.0 asset tree. Real geometry comes from CAD: `tools/cad_to_usd.py` (verified working) takes step/dwg/dxf/sldprt/catpart/jt/stl/gltf via the installed HOOPS converter. ⚠ A raw import is not SimReady — expect wrong units (the STL test came back `metersPerUnit=0.001`), no semantics, no physics.
