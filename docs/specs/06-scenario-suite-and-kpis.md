...
| `KPI-05` | Station-keep error | max/mean deviation (meters) from the commanded hold pose during a screen/confirm pass under wind/wake | `SLICE-2` | **Calm-air baseline measured 2026-07-29**: a PX4-governed Iris held **2.562 m within 43 mm over 35 s** (worst \|vz\| 0.023 m/s), `tools/px4_hover.py`. ⚠ This is the *baseline*, not the KPI: the metric is defined under wind, and no wind field was applied (`FR-12`/`RISK-27`). ⚠ Compute it from Isaac ground truth, **never** from PX4's own estimate — the two disagree by ~0.23 m and the autopilot is the thing under test (`RISK-29`) |
