...
### IF-08 — Canonical site file + `layout.kind` switch (SHIPPED — recorded for traceability)

**Need:** `FR-26` — build the twin from a *real* plant layout, not only a
procedural grid, without touching the interfaces or the `pv:` schema.

**What shipped:**

- **`configs/layouts/<site>.yaml`** — a new, generated config surface describing
  the site at **table level** (one entry per tracker table, not per module), with
  exact survey coordinates in metres plus the CRS. Generated by
  `tools/layout_from_dxf.py`; never hand-edited (regenerate instead).
- **`world/layout_import.py`** — pure-python expansion of tables → one
  `PanelSite` per module. `pyproj` is imported lazily so the geometry path and its
  tests run without it (`NFR-01` preserved).
- **`FarmLayout`** gains `layout.kind: grid | file`, defaulting to **`grid`** so
  every pre-existing config is unaffected (`NFR-04`). Downstream code reads only
  `self.sites`, so seeded faults, `panel_records`, `inspection_targets`,
  `farm_builder` and the KPI harness are all unchanged.
- **`PanelSite`** gains `azimuth_deg` and `tilt_deg` (defaulted, so additive).
  `farm_builder` now orients each panel from these instead of one global
  `panel.tilt_deg` — required because a real plant has per-block orientation.
- **`FarmLayout.bounds()`** — the builder sizes the ground mesh from the actual
  panel bounding box; `rows × pitch` is meaningless for an irregular site.

```yaml
# configs/farm.yaml — opt in to a real site
layout:
  kind: file
  path: configs/layouts/khavda_a10b_block02.yaml
```

**Grid-attribute semantics under `kind: file`** (stated because they are reused,
not re-derived): `rows` = table count, `cols` = max modules per table,
`row_pitch` = median across-row aisle, `col_pitch` = module pitch. The synthetic
`(row, col)` = `(table index, module index)` — needed because
`pv_module.panel_path()` derives the USD prim path from `(row, col)`, so every
panel needs a unique pair even though its real identity is a table ID plus a
module number.
...
torque tube runs north-south and the panel rotates east-west to follow the sun.
There is therefore **no fixed tilt**. `PanelSite.tilt_deg` carries only the
*nominal/stowed* angle, and authoring it as a static rotation is an approximation
that must be declared as one (`NFR-07`), never presented as validated geometry.

**Consequence, and an opportunity:** tracker angle belongs on the **scenario**
axis (`IF-03`), driven by sun position — which also yields a far better `KPI-03`
false-fault stimulus than the turbine-blade geometry `SLICE-3` struggled with:
adjacent tracker rows shade each other predictably at low sun, on the panel
surface, across many panels at once. That directly addresses the KPI-03
denominator caveat (only ~3 of 10 healthy panels carried the earlier shadow).
