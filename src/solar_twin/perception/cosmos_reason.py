"""Cosmos-Reason-style Perception backend (Slice 0 skeleton).

Targets the local Qwen2.5-VL-72B vLLM server (OpenAI-compatible, `:8000` —
see `docs/ENVIRONMENT.md`) as a stand-in for Cosmos Reason. Implementing the
same `Perception` interface as `ground_truth.py` means swapping this in is a
...
class _HttpChatClient:
    """Talks to a real OpenAI-compatible `/v1/chat/completions` endpoint (e.g.
    vLLM serving Qwen2.5-VL). Uses stdlib `urllib` only — no new dependency —
    and only imports/opens a socket when `.complete()` is actually called."""
